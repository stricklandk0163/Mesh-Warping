...
TrackBall::TrackBall(float angularVelocity, bool retainMomentum, const QVector3D& axis, TrackMode mode)
    : m_axis(axis)
...
void TrackBall::push(const QPointF& p, const QMatrix4x4 &)
{
    m_rotation = rotation();
    m_pressed = true;
    m_lastTime = QTime::currentTime();
...
void TrackBall::move(const QPointF& p, const QMatrix4x4 &transformation)
{
    if (!m_pressed)
        return;
...
void TrackBall::release(const QPointF& p, const QMatrix4x4 &transformation)
{
    // Calling move() caused the rotation to stop if the framerate was too low.
    move(p, transformation);
    if (!m_retainMomentum) m_angularVelocity = 0.0f;
...
void TrackBall::addRotation(const QVector3D &pAxis, float pAngle)
{
    m_angularVelocity = 0.0f;
    m_axis = pAxis;
    m_rotation = QQuaternion::fromAxisAndAngle(m_axis, pAngle)*m_rotation;
}

void TrackBall::setRotation(const QVector3D &pAxis, float pAngle)
{
    m_angularVelocity = 0.0f;
    m_axis = pAxis;
